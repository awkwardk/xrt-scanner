...
test('Photo upload to eBay CDN',      has('UploadSiteHostedPictures') && has('function uploadAllPhotos('));
...
test('Photo Ack=Success check',       has("ack === 'Success'") && has('photo upload Ack='));
test('Photo read from file',          has('fs.readFile(photoPath'));
test('Photo fallback to server URL',  has('using fallback URL') && has('/api/photo/'));
